...
- **`ContinuousAsyncWrapper`**: caller-selected deposit ids from one shared pool with a `minDecoys` lower bound; matched deposit amounts are rewritten to encrypted zero (per-deposit nullifier).
